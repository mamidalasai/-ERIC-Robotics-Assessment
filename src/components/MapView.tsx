...
export default function MapView({ isPiP, onClick }: Props) {
  ...
  return (
    <div
      className={`absolute transition-all duration-500 ease-in-out overflow-hidden bg-[#f0f2f5] flex items-center justify-center ${isPiP ? pipClasses : fullscreenClasses}`}
      onClick={isPiP ? onClick : undefined}>
      
      <img
        className="w-full h-full object-cover"
        src="/Gemini_Generated_Image_pfpf35pfpf35pfpf.png"
        alt="Map View" />
      

      {/* Hover Overlay - Only visible when in PiP mode */}
      {isPiP &&
      <div className="absolute inset-0 bg-black/60 opacity-0 group-hover:opacity-100 transition-opacity duration-300 flex items-center justify-center backdrop-blur-sm z-50">
          <span className="text-white font-semibold text-sm tracking-wide bg-black/50 px-4 py-2 rounded-full">
            Click to enter map view
          </span>
        </div>
      }
    </div>);
